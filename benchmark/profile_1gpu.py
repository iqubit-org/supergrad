#!/usr/bin/env python3
"""
1-GPU Profiler Script
Sets CUDA_VISIBLE_DEVICES=0 before JAX import for true isolation
"""
...
print("   Expected GPUs: 1 (GPU_0 only)")
...
if len(jax.devices()) != 1:
    print(f"   ❌ GPU isolation failed - expected 1 GPU, got {len(jax.devices())}")
    sys.exit(1)
else:
    print("   ✅ GPU isolation successful: 1 device visible")
...
# Configuration
n_qubit = 8
print(f"\n🧪 Starting 1-GPU Profiling with n_qubit={n_qubit}")
print("=" * 60)
...
tensorboard_log_dir = setup_tensorboard_logging(1)
...
try:
    ...
    result, trace_dir = run_with_jax_profiler(
        ...
        "lcam_trace", 
        tensorboard_log_dir,
        "LCAM gradient computation on 1 GPU"
    )
...
try:
    ...
    result, trace_dir = run_with_jax_profiler(
        ...
        "tad_trace", 
        tensorboard_log_dir,
        "TAD gradient computation on 1 GPU"
    )
...
# Save results
output_file = 'profile_1gpu_results.json'
with open(output_file, 'w') as f:
    json.dump(results, f, indent=2)

print(f"\n💾 Results saved to: {output_file}")
print("🎯 1-GPU profiling completed!")
print("=" * 60)
