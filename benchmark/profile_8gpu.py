#!/usr/bin/env python3
"""
8-GPU Profiler Script
Sets CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 before JAX import for true isolation
"""
...
print("   Expected GPUs: 8 (GPU_0, GPU_1, GPU_2, GPU_3, GPU_4, GPU_5, GPU_6, GPU_7)")
...
if len(jax.devices()) != 8:
    print(f"   ❌ GPU isolation failed - expected 8 GPUs, got {len(jax.devices())}")
    sys.exit(1)
else:
    print("   ✅ GPU isolation successful: 8 devices visible")
...
def verify_gpu_participation():
    ...
    try:
        ...
        sharded_array = jax.device_put_sharded(
            [test_array[:250, :250], test_array[250:500, :250], 
             test_array[500:750, :250], test_array[750:, :250],
             test_array[:250, 250:500], test_array[250:500, 250:500],
             test_array[500:750, 250:500], test_array[750:, 250:500]],
            jax.devices()
        )
...
# Configuration
n_qubit = 8
print(f"\n🧪 Starting 8-GPU Profiling with n_qubit={n_qubit}")
print("=" * 60)
...
tensorboard_log_dir = setup_tensorboard_logging(8)
...
try:
    ...
    result, trace_dir = run_with_jax_profiler(
        ...
        "lcam_trace", 
        tensorboard_log_dir,
        "LCAM gradient computation on 8 GPUs"
    )
...
try:
    ...
    result, trace_dir = run_with_jax_profiler(
        ...
        "tad_trace", 
        tensorboard_log_dir,
        "TAD gradient computation on 8 GPUs"
    )
...
# Save results
output_file = 'profile_8gpu_results.json'
with open(output_file, 'w') as f:
    json.dump(results, f, indent=2)

print(f"\n💾 Results saved to: {output_file}")
print("🎯 8-GPU profiling completed!")
print("=" * 60)
