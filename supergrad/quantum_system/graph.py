...
from .artificial_atom import Fluxonium, Transmon, Resonator
from .interaction import InteractingSystems, InteractionTerm, parse_interaction
...
class BaseGraph(nx.Graph):
    ...
    def convert_graph_to_pulse_parameters_haiku(self):
    ...
    def convert_graph_to_quantum_system(self,
                                        ...
                                        **kwargs):
        """Convert a networkX graph to AQGrad quantum system.

        Args:
            enable_var: whether adding device parameters variance or not
        ...
        return InteractingSystems(subsystem_list, inter_list)

    def convert_graph_to_pulse_lst(self, hilbert_space: InteractingSystems,
                                   **kwargs):
    ...
    def update_params(self,
        ...
        for node_name in NodeView(self):
            node = NodeView(self)[node_name]
            for k, v in dict(node).items():
                if str(k).startswith('pulse'):
                    pulse_type = v['pulse_type']
                    v.update(pulse_dict[_parse_pulse_name(
                        node_name, k, pulse_type)])
