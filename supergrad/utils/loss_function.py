...
from supergrad.quantum_system import (Fluxonium, Resonator, InteractingSystems,
                                      parse_interaction)
...
def _cal_transmission_vs_phi(phi,
    ...
    def calc_transmission_vs_phi(phi):
        ...
        # now instance InteractingSystems to handle components and InteractionTerms
        # one can set module name here
        hilbertspace = InteractingSystems(subsystem_list,
                                          inter_term,
...
def loss_kl_div(xs, ys, exp_array, plot=False):
    ...
    if plot:
        ...
        axs[0].set_title(r'$\mathbf{P}(x, \varepsilon)$')
    ...
    if plot:
        ...
        axs[1].set_title(r'$\mathbf{Q}(x, \varepsilon)$')
