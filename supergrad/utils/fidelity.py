...
import supergrad
from supergrad.quantum_system.graph import (parse_pre_comp_name,
...
def estimate_vz(
    ...
    u_computed = evo.eigen_basis(
        hk.data_structures.merge(evo.static_params, params), transform_matrix)
...
def estimate_vsq(
    ...
    if isinstance(initial_guess.get('single_q_compensation', None), dict):
...
def compute_fidelity_with_1q_rotation_axis(u_target,
    ...
    Args:
        ...
        options (dict): additional keyword arguments to be passed to the `jaxopt`
            optimizer.
        n_qubit: number of qubit in the composited quantum system.
    """
