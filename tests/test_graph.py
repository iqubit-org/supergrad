...
def graph_to_quantum_system(graph: BaseGraph = graph,
                            enable_var=False,
                            share_params=False):
...
def test_enable_variable():
    hk_0, params_0 = graph_to_quantum_system(enable_var=False)
    hk_1, params_1 = graph_to_quantum_system(enable_var=True,
                                             share_params=False)
    hk_2, params_2 = graph_to_quantum_system(enable_var=True, share_params=True)
...
def test_update_params(share_params=False):
    ...
    static_params = qubit_subset_temp.convert_graph_to_parameters_haiku(
        share_params=share_params)
    # save parameters to json after optimizing
    save_params = json.dumps(convert_to_json_compatible(static_params))
